...
.order-card { background: #111; padding: 25px; border-radius: 10px; margin-bottom: 25px; border-left: 4px solid #FFD93D; }
.order-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; }
.order-id { font-size: 18px; font-weight: bold; color: #FFD93D; }
.order-status { padding: 6px 15px; border-radius: 20px; font-size: 12px; font-weight: bold; }
.status-pending { background: #FFD93D; color: #000; }
.status-processing { background: #6BCF7F; color: #000; }
.status-completed { background: #4CAF50; color: #fff; }

.order-details { display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 15px; font-size: 14px; }
.detail-item { color: #ccc; }
.detail-item strong { color: #FFD93D; }

.order-items { background: #1a1a1a; padding: 15px; border-radius: 8px; margin-bottom: 15px; }
.item { display: flex; justify-content: space-between; padding: 8px 0; color: #ccc; font-size: 14px; }

.empty-orders { text-align: center; padding: 60px 20px; }
.empty-orders i { font-size: 80px; color: #FFD93D; margin-bottom: 20px; }
.empty-orders p { color: #ccc; margin-bottom: 20px; }
...
<div class="container">
    ...
    <?php if (isset($_GET['order_confirmed']) && $_GET['order_confirmed'] === 'true'): ?>
        <div style="background: #4CAF50; color: white; padding: 20px; border-radius: 10px; margin-bottom: 30px; text-align: center; font-weight: bold; font-size: 18px;">
